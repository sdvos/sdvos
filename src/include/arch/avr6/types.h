...
/**
 * @file   src/include/arch/avr6/types.h
 * @author Ye Li (liye@sdvos.org)
 * @brief  AVR6 Type Definitions
 */
#ifndef _AVR6_TYPES_H_
#define _AVR6_TYPES_H_

/* Size of task context on stack excluding PC (3 bytes) */
/* A context flag is also included */
#define CONTEXT_SIZE    0x25

/* Field offsets in processor context structure */
#define CONTEXT_OFFSET                         0x4
#define CONTEXT_PCE_OFFSET          CONTEXT_OFFSET
#define CONTEXT_PCH_OFFSET    (CONTEXT_OFFSET + 1)
#define CONTEXT_PCL_OFFSET    (CONTEXT_OFFSET + 2)
#define CONTEXT_SREG_OFFSET   (CONTEXT_OFFSET + 3)
#define CONTEXT_REGS_OFFSET   (CONTEXT_OFFSET + 4)
#define CONTEXT_FLAG_OFFSET   (CONTEXT_OFFSET + 36)
...
/**
 * @union task_context
 * @brief AVR6 processor context
 *
 * AVR6 general registers, Program Counter, Status Register
 * and context flag.
 */
typedef union {
  /**
   * @struct regs_struct
   * @brief AVR6 registers and flag
   */
  struct regs_struct {
    uint8_t pce;    /**< Extended byte of PC */
